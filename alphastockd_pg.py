...
class PG(object):
    ...
    def choose_action(self, observation):
        ...
        winner_port, loser_port = action_prob[:G], action_prob[-G:]
        winner_idx, loser_idx = sorted_indices[:G], sorted_indices[-G:]

        winner_stock_id, loser_stock_id = np.random.choice(winner_idx, p=winner_port), np.random.choice(loser_idx, p=loser_port)
        ...
        action = [0]*len(sorted_indices)


        # for i in range(len(action_shuffle)):
        #     action[sorted_indices[i]] = action_shuffle[i]
        action[winner_stock_id], action[loser_stock_id] = 1, -1
